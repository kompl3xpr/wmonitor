...
pub struct FiefLock(FiefId, pub Arc<Mutex<()>>);

impl Drop for FiefLock {
    fn drop(&mut self) {
        FIEF_LOCKS.remove(&self.0);
    }
}
...
pub fn get_fief_lock(fief: FiefId) -> FiefLock {
    ...
    FiefLock(fief, FIEF_LOCKS.get(&fief).unwrap().clone())
}

macro_rules! lock_fief {
    ($fief_id:expr) => {
        let _lock_of_fief = $crate::core::lock::get_fief_lock($fief_id);
        let _lock_guard_of_fief = _lock_of_fief.1.lock().await;
    };
}
